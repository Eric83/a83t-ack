...
static int diswitch_status = 0;
static ssize_t gpio_keys_show_diswitch(struct device *dev, struct device_attribute *attr, char *buf)
{
  return sprintf(buf, "%d\n", diswitch_status);
}
static DEVICE_ATTR(diswitch, 0666, gpio_keys_show_diswitch, NULL);
...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
  ...
  if(button->gpio == GPIO_DISWITCH)
    diswitch_status = state;
	input_sync(input);
}
