...
printk("ft5x ctp_detect found_tp = %d\n", found_tp);
	if(found_tp == 1)
	  return -ENODEV;
...
static int ft5x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
 ...
 {
    char gpio_irq_name[8];
    long unsigned int	config;
    sunxi_gpio_to_name(CTP_IRQ_NUMBER,gpio_irq_name);
    config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_PUD, 1);
    pin_config_set(SUNXI_PINCTRL, gpio_irq_name, config);
 }
