...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
 ...
 //       ret = gtp_i2c_read_dbl_check(ts->client, GTP_REG_SENSOR_ID, &sensor_id, 1);
	dprintk(DEBUG_INIT,"CTP name : %s\n",config_info.name);
        if (!strcmp(config_info.name,"peite_gt9xx_7")){
            sensor_id = 0;
            dprintk(DEBUG_INIT,"gt9xx:sensor_id = %d\n",sensor_id);
			
		} else if (!strcmp(config_info.name,"jinuo_gt9xx_7")){
            sensor_id = 1;
            dprintk(DEBUG_INIT,"gt9xx:sensor_id = %d\n",sensor_id);
			
		} else if (!strcmp(config_info.name,"jinuo_gt9xx_10")){
            sensor_id = 2;
            dprintk(DEBUG_INIT,"gt9xx:sensor_id = %d\n",sensor_id);
			
		} else if (!strcmp(config_info.name,"gt9xxf_dzc_101")){
            sensor_id = 3;
            dprintk(DEBUG_INIT,"gt9xx:sensor_id = %d\n",sensor_id);
...
{   
        if (!strcmp(config_info.name,"jinuo_gt9xx_7"))
        {
          config_info.revert_x_flag = 1;
          config_info.revert_y_flag = 1;
