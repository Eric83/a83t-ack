...
static inline __sum16 tcp_v6_check(int len, const struct in6_addr *saddr, const struct in6_addr *daddr, __wsum base)
{
...
extern char mac_str[24];
extern void geth_chip_hwaddr(u8 *addr);
static inline void set_ethernet_addr(struct r8152 *tp)
{
...

  {
    char *p = mac_str;
    printk("mac_str = %s\n", mac_str);
    if(strncmp(mac_str, "00:00:00:00:00:00", 17) != 0)
    {
      int i;
  		for(i=0; i<ETH_ALEN; i++, p++)
  			node_id[i] = simple_strtoul(p, &p, 16);
  	  node_id[0] = node_id[0] - 2;
  		printk("rtl8152_mac: %02x:%02x:%02x:%02x:%02x:%02x\n",node_id[0], node_id[1], node_id[2], node_id[3], node_id[4], node_id[5]);
  	}
    if (!is_valid_ether_addr(node_id))
		  geth_chip_hwaddr(node_id);
    if (!is_valid_ether_addr(node_id))
...
out1:
  {
    static int link_stats = -1;
    if((test_bit(RTL8152_UNPLUG, &tp->flags)) != link_stats)
    {
    	if (test_bit(RTL8152_UNPLUG, &tp->flags))
    		printk("%s: link down\n", tp->netdev->name);
    	else
    		printk("%s: link up, %dMbps, full-duplex\n", tp->netdev->name, (rtl8152_get_speed(tp) & _10bps) ? 10 : 100);
      link_stats = test_bit(RTL8152_UNPLUG, &tp->flags);
    }
  }
	usb_autopm_put_interface(tp->intf);
}
...
static void r8152b_init(struct r8152 *tp)
{
  ...
  ocp_write_word(tp, MCU_TYPE_PLA, PLA_LEDSEL, 0x40F7);
	if (tp->version == RTL_VER_01) {
