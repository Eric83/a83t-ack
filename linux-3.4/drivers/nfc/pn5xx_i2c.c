...
//  int i = 0;
//
//  printk("%s  : reading %zu bytes: ", __func__, count);
//  for(i=0; i<ret; i++)
//    printk("%02x ", tmp[i]);
//  printk("\n");
//  
...
//  int i = 0;
//
//  printk("%s : writing %zu bytes: ", __func__, count);
//  for(i=0; i<count; i++)
//    printk("%02x ", tmp[i]);
//  printk("\n");
//  
...
//	pn54x_dev->sevdd_reg = pdata->sevdd_reg;
	...
	pn54x_dev->irq_gpio = GPIO_NFC_IRQ;
	pn54x_dev->firm_gpio = GPIO_NFC_FIRM;
	pn54x_dev->ven_gpio = GPIO_NFC_VEN;
	client->irq = gpio_to_irq(pn54x_dev->irq_gpio);
...
static int nfc_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	if(adapter->nr == NFC_TWI_NUM){
    strlcpy(info->type, "pn544", I2C_NAME_SIZE);

    gpio_request(GPIO_NFC_IRQ, "PN544_INT");
    gpio_request(GPIO_NFC_FIRM, "PN544_FIRM");
    gpio_request(GPIO_NFC_VEN, "PN544_VEN");
	  gpio_direction_output(GPIO_NFC_FIRM, 0);
	  gpio_direction_output(GPIO_NFC_VEN, 1);
    while(retry_time-- > 0)
    {
//    for test should received 0x03   reset_cmd
      char tmp[MAX_BUFFER_SIZE];
      int i,ret;
      tmp[0] = 0x05;
      tmp[1] = 0xF9;
      ...
      tmp[5] = 0xE5;
  	  printk("nfc_detect %d\n", retry_time);
    	ret = i2c_master_send(client, tmp, 6);
    	if(ret <= 0)
    	{
    	  msleep(100);
    	  continue;
...
static const unsigned short normal_i2c[2] = {0x28,I2C_CLIENT_END};
