...
static int rtc_fetch_sysconfig_para(void)
{
  ...
  script_item_u val;
	...
	printk("========RTC Inital ===================\n");
	type = script_get_item("rtc_para", "rtc_used", &val);
  if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
    printk("rtc: script_get_item return type err\n");
	...
	if(1 == device_used){
  	type = script_get_item("rtc_para", "rtc_name", &val);
    ...
    name = val.str;
  	...
  	type = script_get_item("rtc_para", "rtc_twi_addr", &val);
    if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
      printk("rtc: script_get_item return type err\n");
      goto script_parser_fetch_err;
  	}

		u_i2c_addr.dirty_addr_buf[0] = (__u32)(val.val);
		u_i2c_addr.dirty_addr_buf[1] = I2C_CLIENT_END;
		printk("%s: after: rtc_twi_addr is 0x%x, dirty_addr_buf: 0x%hx. dirty_addr_buf[1]: 0x%hx \n", __func__, twi_addr, u_i2c_addr.dirty_addr_buf[0], u_i2c_addr.dirty_addr_buf[1]);

  	type = script_get_item("rtc_para", "rtc_twi_id", &val);
    if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
      printk("rtc: script_get_item return type err\n");
      goto script_parser_fetch_err;
  	}
...
read_retry:
	...
	tm->tm_year += 70;
	if (tm->tm_year < 70)
		tm->tm_year = 70;	/* assume we are in 1970...2069 */
...
static int pcf8563_set_datetime(struct i2c_client *client, struct rtc_time *tm)
{
    ...
    */
	...
	if(leap_year > 2069 || leap_year < 1970) {
		dev_err(&client->dev, "rtc only supports 63 (1970～2069) years\n");
		return -EINVAL;
	}
...
int pcf8563_get_time(struct rtc_time *tm)
{
  if(this_client !=NULL)
