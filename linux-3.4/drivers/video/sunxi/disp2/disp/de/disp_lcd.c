...
static s32 lcd_parse_panel_para(u32 disp, disp_panel_para * info)
{
    ...
    info->backlight_max_limit = 255;
    ret = disp_sys_script_get_item(primary_key, "lcd_pwm_max_limit", &value, 1);
    if(ret == 1)
    {
        info->backlight_max_limit = (value > 255)? 255:value;
    }
    return 0;
}
...
//lcd_power0
	for(i=0; i<LCD_POWER_NUM; i++)
	{
		...
		ret = disp_sys_script_get_item(primary_key,sub_name, (int *)(lcd_cfg->lcd_regu[i]), sizeof(disp_gpio_set_t)/sizeof(int));
		if(ret == 3) {
			/* gpio */
...
s32 disp_lcd_set_bright(struct disp_device *lcd, u32 bright)
{
  ...
  if((lcdp->panel_info.backlight_max_limit) < 255)
  {
    u32 bright_tmp = backlight_bright;
    backlight_bright = (bright_tmp * (lcdp->panel_info.backlight_max_limit)) / 255;
  }
...
static s32 disp_lcd_enable(struct disp_device* lcd)
{
  ...
  if((lcd->disp == 0) && (lcd_first_init == 1))
  {
  	lcd_clk_disable(lcd);
