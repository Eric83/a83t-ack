...
static ssize_t disp_lcd_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", lcd_status);
}
static ssize_t disp_lcd_status_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
  __u32 pin_config;
  if(lcd_status == 1)
  {
    pin_config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_FUNC, 0);
    pin_config_set(SUNXI_PINCTRL, "PD18", pin_config);
    pin_config_set(SUNXI_PINCTRL, "PD19", pin_config);
    pin_config_set(SUNXI_PINCTRL, "PD20", pin_config);
    pin_config_set(SUNXI_PINCTRL, "PD21", pin_config);
    pin_config_set(SUNXI_PINCTRL, "PD22", pin_config);
    pin_config_set(SUNXI_PINCTRL, "PD23", pin_config);
    pin_config_set(SUNXI_PINCTRL, "PD24", pin_config);
    pin_config_set(SUNXI_PINCTRL, "PD25", pin_config);
    pin_config_set(SUNXI_PINCTRL, "PD26", pin_config);
    pin_config_set(SUNXI_PINCTRL, "PD27", pin_config);
//    gpio_direction_output(GPIO_LCD1_VCC, 0);
    lcd_status = 0;
  }
  else 
  {
    pin_config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_FUNC, 3);
    pin_config_set(SUNXI_PINCTRL, "PD18", pin_config);
    pin_config_set(SUNXI_PINCTRL, "PD19", pin_config);
    pin_config_set(SUNXI_PINCTRL, "PD20", pin_config);
    pin_config_set(SUNXI_PINCTRL, "PD21", pin_config);
    ...
    pin_config_set(SUNXI_PINCTRL, "PD26", pin_config);
    pin_config_set(SUNXI_PINCTRL, "PD27", pin_config);
//    gpio_direction_output(GPIO_LCD1_VCC, 1);
    lcd_status = 1;
  }
  return count;
}
static DEVICE_ATTR(lcd_status, 0666, disp_lcd_status_show, disp_lcd_status_store);
