...
static __s32 main_Hpd_Check(void)
{
	...
	if(times >= 3)
  {
    ...
    {
      int ret;
      printk("HDMI FORCE OUTPUT\n");

#ifdef CONFIG_VIDEO_LT8619B
#ifdef CONFIG_VIDEO_LT8619B_INIT
      do{
        printk("lt8619b_i2c_init\n");
        ret = lt8619b_i2c_init();
        if(ret == 0)
          break;
        printk("lt8619b_i2c_init retry\n");
...
#else
      printk("lt8619b_spi_flash_init\n");
      gpio_direction_output(GPIO_LCD2_BACKLIGHT, 1);
#endif
#endif
      hdmi_my_reset = 0;
    }
