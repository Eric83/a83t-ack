...
#ifdef CONFIG_SND_SOC_SPEAKER_FILP
  #define SPEAK_POWER_ON  0
  #define SPEAK_POWER_OFF 1
#else
  #define SPEAK_POWER_ON  1
  #define SPEAK_POWER_OFF 0
#endif
				...
				int event)
{
	...
	case	SND_SOC_DAPM_POST_PMU:
		...
		msleep(30);
		gpio_set_value(item.gpio.gpio, SPEAK_POWER_ON);
		break;
	case	SND_SOC_DAPM_PRE_PMD :
		AC100_DBG("[speaker close ]%s,line:%d\n",__func__,__LINE__);
		gpio_set_value(item.gpio.gpio, SPEAK_POWER_OFF);
		if (drc_used) {
			drc_enable(codec,0);
		}
	default:
...
#ifdef CONFIG_SND_SOC_HP_ADCMIC
    gpio_set_value(item.gpio.gpio, SPEAK_POWER_ON);
#endif
		break;
	case SND_SOC_DAPM_PRE_PMD:
...
static int ac100_codec_probe(struct snd_soc_codec *codec)
{
	...
	*/
	...
	/* request virq, set virq type to high level trigger */
//	ret = devm_request_irq(codec->dev, ac100->virq, audio_hmic_irq, IRQF_TRIGGER_FALLING, "SWTICH_EINT", ac100);
#ifdef CONFIG_SND_SOC_GPIO_DETECT
	ret = devm_request_irq(codec->dev, ac100->virq, audio_hmic_irq, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING , "SWTICH_EINT", ac100);
#else
	ret = devm_request_irq(codec->dev, ac100->virq, audio_hmic_irq, IRQF_TRIGGER_FALLING, "SWTICH_EINT", ac100);
#endif
	if (IS_ERR_VALUE(ret)) {
