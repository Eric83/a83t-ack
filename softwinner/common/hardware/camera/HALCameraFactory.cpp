          ...
          mConstructedOK(false)
{
    ...
    /* Create the cameras */
	for (int id = 0; id < (MAX_NUM_OF_CAMERAS + 1); id++)
	{
...
//		ret = access(dev_node, F_OK);
    ret = 0;
		if(ret == 0)
		{
			...
			mHalCameraInfo[1].orientation	= mCameraConfig[2]->getCameraOrientation();
			mHalCameraInfo[1].fast_picture_mode	= mCameraConfig[2]->supportFastPictureMode();
			mHardwareCameras[1]->mCameraConfig = mCameraConfig[2];
		}
	}
...
//			sprintf(dev_node, "/dev/video%d", i);
			sprintf(dev_node, "/dev/video%d", i+1);
			ret = access(dev_node, F_OK);
			if(ret == 0)
			{
		...
		if (mRemovableCamerasNum == 1)
		{
			...
			mHalCameraInfo[0].orientation	= mCameraConfig[2]->getCameraOrientation();;
			mHalCameraInfo[0].fast_picture_mode	= mCameraConfig[2]->supportFastPictureMode();;
			mHardwareCameras[0]->mCameraConfig = mCameraConfig[2];
		}
